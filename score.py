...
# Guard against an inherited MPLBACKEND environment variable (e.g. left over
# from a Jupyter/VS Code session) pointing at an inline backend that only
# exists inside a live notebook kernel. If left in place, matplotlib raises
# at import time before this script's own matplotlib.use("Agg") call below
# ever runs. Safe to strip unconditionally - this script always wants the
# non-interactive Agg backend for saving PNGs to disk.
